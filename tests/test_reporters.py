...
class TestGenerateAnomaliesCsv:
    def test_empty_anomalies_produces_header_only(self):
        csv = generate_anomalies_csv([])
        lines = csv.strip().split("\r\n")
        assert len(lines) == 1
        assert "IP" in lines[0]

    def test_single_anomaly_produces_correct_row(self):
        anomalies = [AnomalyResult(ip="10.0.0.1", total_errors=5, client_errors=3, server_errors=2, total_requests=10, error_rate=0.5)]
        csv = generate_anomalies_csv(anomalies)
...
class TestGeneratePerformanceCsv:
    def test_empty_performance_produces_header_only(self):
        csv = generate_performance_csv([])
        lines = csv.strip().split("\r\n")
        assert len(lines) == 1

    def test_performance_row_with_times(self):
        results = [PerformanceResult(endpoint="GET /api", request_count=5, avg_response_time=0.1234, min_response_time=0.01, max_response_time=0.5)]
        csv = generate_performance_csv(results)
        assert "GET /api" in csv
        assert "5" in csv
...
class TestGenerateSecurityCsv:
    def test_empty_alerts_produces_header_only(self):
        csv = generate_security_csv([])
        lines = csv.strip().split("\r\n")
        assert len(lines) == 1

    def test_security_alert_row(self):
        alerts = [SecurityAlert(path="/search?q=1 OR 1=1", method="GET", ip="10.0.0.1", pattern_type="SQL Injection", matched_content="OR")]
        csv = generate_security_csv(alerts)
        assert "SQL Injection" in csv
        assert "10.0.0.1" in csv


class TestGenerateFullReport:
    def test_full_report_with_all_sections(self):
        anomalies = [AnomalyResult(ip="10.0.0.1", total_errors=5, client_errors=5, server_errors=0, total_requests=10, error_rate=0.5)]
        performance = [PerformanceResult(endpoint="GET /api", request_count=3, avg_response_time=0.2, min_response_time=0.1, max_response_time=0.3)]
        security = [SecurityAlert(path="/xss", method="GET", ip="10.0.0.1", pattern_type="XSS", matched_content="<script>")]
    ...
    def test_full_report_shows_no_anomalies_when_empty(self):
