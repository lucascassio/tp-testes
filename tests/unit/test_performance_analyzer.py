...
class TestPerformanceAnalyzer:
    def test_average_response_time_single_endpoint(self, make_entry, performance_analyzer):
        entries = [
        ...
        results = performance_analyzer.analyze(entries)

        assert len(results) == 1, f"Expected 1 endpoint group, got {len(results)}"
    ...
    def test_multiple_endpoints_grouped_separately(self, make_entry, performance_analyzer):
        entries = [
            make_entry(path="/api/users", request_time=0.1),
            make_entry(path="/api/users", request_time=0.3),
        ...
        assert len(results) == 2, f"Expected 2 endpoint groups, got {len(results)}"
        endpoints = {r.endpoint for r in results}
        assert endpoints >= {"GET /api/users", "GET /api/products"}, f"Missing endpoints: {endpoints}"

    def test_empty_entries_returns_empty_list(self, performance_analyzer):
        assert performance_analyzer.analyze([]) == []

    def test_single_entry_performance(self, make_entry, performance_analyzer):
    ...
    def test_zero_response_times(self, make_entry, performance_analyzer):
        entries = [
    ...
    def test_sorted_by_avg_descending(self, make_entry, performance_analyzer):
        entries = [
        ...
        assert results[0].endpoint == "GET /slow", f"Expected slowest first, got {results[0].endpoint}"
    ...
    def test_different_methods_same_path_grouped_differently(self, make_entry, performance_analyzer, method, request_time):
        entries = [
        ...
        assert len(results) == 3, f"Expected 3 groups (one per method), got {len(results)}"
        endpoints = {r.endpoint for r in results}
        assert endpoints >= {"GET /api/data", "POST /api/data", "DELETE /api/data"}
